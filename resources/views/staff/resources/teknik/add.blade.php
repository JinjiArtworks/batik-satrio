...
@section('content')
    <main class="h-full overflow-y-auto">
                        ...
                        <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                            @foreach ($teknik as $item)
                                <tr class="text-gray-700 dark:text-gray-400">
                                    ...
                                    <td class="px-4 py-3 text-sm">
                                        <form method="GET"
                                            action="{{ route('resources.edit-teknik', ['id' => $item->id]) }}">
                                    ...
                                    <td class="px-4 py-3 text-sm">
                                        <form method="GET"
                                            action="{{ route('resources.delete-teknik', ['id' => $item->id]) }}">
                                            <button type="submit"
        ...
        <div class="container px-6 mx-auto grid">
            <h2 class="mt-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
                Tambah Teknik Pembuatan
            </h2>
            <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                <form method="POST" action="{{ route('resources.store-teknik') }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <button type="submit"
                        class="confirmTeknik px-3 py-1 mt-4 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                        Submit
                    </button>
                </form>
            </div>
...
@section('script')
    <script>
        ...
        $('.confirmTeknik').click(function(event) {
            event.preventDefault();
            var form = $(this).closest("form");
            Swal.fire({
